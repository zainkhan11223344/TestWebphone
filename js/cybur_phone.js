...
// On-hook Agent Class
class OnhookAgent {
	constructor(agentId, phoneNumber, vicidialUrl) {
	...
	init() {
	  ...
	  const incomingCallButton = document.getElementById('incoming-call-button');
	  incomingCallButton.addEventListener('click', () => {
		this.answerIncomingCall();
	  });
  
	  // Add event listener for declining calls
	...
	answerIncomingCall() {
	  // Make API call to Vicidial to answer the call
	  fetch(`${this.vicidialUrl}/api/agent/answer_call`, {
		method: 'POST',
		headers: {
	  ...
	  .then(data => {
		// Update the UI to show the call is being answered
		const callStatusElement = document.getElementById('call-status');
		callStatusElement.textContent = 'Answering...';
	  })
	  .catch(error => {
		console.error(error);
	  });
	...
	declineIncomingCall() {
	  // Make API call to Vicidial to decline the call
	  fetch(`${this.vicidialUrl}/api/agent/decline_call`, {
		method: 'POST',
		headers: {
		  'Content-Type': 'application/json'
		},
	  ...
	  .then(response => response.json())
	  .then(data => {
		// Update the UI to show the call is being declined
		const callStatusElement = document.getElementById('call-status');
		callStatusElement.textContent = 'Declined';
	  })
	  .catch(error => {
		console.error(error);
	  });
  ...
  // Initialize the on-hook agent
  const onhookAgent = new OnhookAgent(myUserAgent, '1234567890', vicidialUrl);
  onhookAgent.init();

  // Trigger audio alert or ringtone when inbound call is received
function playRingtone() {
	var audio = new Audio('/sounds/ringing.mp3');
	audio.play();
  }
  
  // Call the playRingtone function when an inbound call is received
  vicidial.on('inboundCallReceived', playRingtone);

  // Function to display pop-up for answering or declining inbound call
function displayCallPopup(callData) {
	// Create a pop-up container element
	var popupContainer = document.createElement('div');
	popupContainer.innerHTML = `
	  <h2>Inbound Call from ${callData.callerId}</h2>
	  <p>Would you like to answer or decline this call?</p>
	  <button id="answerCall">Answer</button>
	  <button id="declineCall">Decline</button>
	`;
  
	// Add event listeners to the answer and decline buttons
	document.getElementById('dial_iconl').addEventListener('click', function() {
	  // Answer the call using Vicidial's API
	  vicidial.answerCall(callData.callId);
	});
  
	document.getElementById('hangup_icon').addEventListener('click', function() {
	  // Decline the call using Vicidial's API
	  vicidial.declineCall(callData.callId);
	});
  
	// Add the pop-up container to the page
	document.body.appendChild(popupContainer);
  }
  
  // Listen for the inboundCallReceived event
  vicidial.on(debugOutdebugOut( 'Session Refer Event Fired'), function(callData) {
	displayCallPopup(callData);
  });
